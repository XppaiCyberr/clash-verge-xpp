...
import { DiscordRpcViewer } from "./mods/discord-rpc-viewer";
...
const SettingVergeAdvanced = ({ onError: _ }: Props) => {
  ...
  const discordRpcRef = useRef<DialogRef>(null);
  ...
  const onDiscordRpcChange = useLockFn(async (enabled: boolean) => {
    try {
      await patchVerge({ enable_discord_rpc: enabled });
  ...
  return (
    <SettingList title={t("settings.components.verge.advanced.title")}>
      ...
      <DiscordRpcViewer ref={discordRpcRef} />
      ...
      <SettingItem
        label={t("settings.components.verge.advanced.fields.discordRpc")}
        extra={
          <>
            <TooltipIcon
              title={t("settings.components.verge.advanced.tooltips.discordRpc")}
              sx={{ opacity: "0.7" }}
            />
            <TooltipIcon
              icon={Settings}
              onClick={() => discordRpcRef.current?.open()}
              sx={{ opacity: "0.7" }}
            />
          </>
        }
      >
        <Switch
          checked={verge?.enable_discord_rpc ?? false}
          onChange={(_, c) => onDiscordRpcChange(c)}
        />
      </SettingItem>
